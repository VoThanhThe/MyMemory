...
class WalletConnectConnector extends _wagmi.Connector {
  id = 'walletConnect';
  name = 'WalletConnect';
  ready = true;
  constructor(config) {
  ...
  async connect({
    ...
    try {
      ...
      if (!targetChainId) {
        const store = await _scaffoldUtilsReactNative.StorageUtil.getItem(STORE_KEY);
      ...
      if (!provider.session || isChainsStale) {
        ...
        this._setRequestedChainsIds(this.chains.map(({
          id
        }) => id));
      }
  ...
  async switchChain(chainId) {
    ...
    if (!chain) throw new _viem.SwitchChainError(new Error('chain not found on connector.'));
    try {
      const provider = await this.getProvider();
  ...
  async _initProvider() {
    ...
    if (defaultChain) {
      ...
      this._provider = await _ethereumProvider.EthereumProvider.init({
        showQrModal: false,
        qrModalOptions: undefined,
        projectId,
        optionalMethods: _ethereumProvider.OPTIONAL_METHODS,
        optionalEvents: _ethereumProvider.OPTIONAL_EVENTS,
        chains: [defaultChain],
        optionalChains: optionalChains.length ? optionalChains : undefined,
  ...
  _setRequestedChainsIds(chains) {
    _scaffoldUtilsReactNative.StorageUtil.setItem(REQUESTED_CHAINS_KEY, chains);
  }
  async _getRequestedChainsIds() {
    const requestedChains = await _scaffoldUtilsReactNative.StorageUtil.getItem(REQUESTED_CHAINS_KEY);
  ...
  _getNamespaceChainsIds() {
    ...
    const normalizedNamespaces = (0, _utils.normalizeNamespaces)(namespaces);
