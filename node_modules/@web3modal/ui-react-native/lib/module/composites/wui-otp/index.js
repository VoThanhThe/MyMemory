import { createRef, useMemo, useState } from 'react';
import { View } from 'react-native';
import { InputNumeric } from '../wui-input-numeric';
import styles from './styles';
export function Otp({
  length,
  style,
  onChangeText
}) {
  const [value, setValue] = useState([]);
  const refArray = useMemo(() => Array.from({
    length
  }).map(() => /*#__PURE__*/createRef()), [length]);
  const _onChangeText = (text, index) => {
    const newValue = [...value.slice(0, index), text, ...value.slice(index + 1)];
    setValue(newValue);
    onChangeText?.(newValue.join(''));
    if (text.length === 1 && index < length - 1) {
      refArray[index + 1]?.current?.focus();
    }
  };
  const onKeyPress = (e, index) => {
    const currentValue = value[index] || '';
    if (e.nativeEvent.key !== 'Backspace' && currentValue && index !== length - 1) {
      refArray[index + 1]?.current?.focus();
      return;
    }
    if (e.nativeEvent.key === 'Backspace' && index !== 0) {
      if (!currentValue) {
        refArray[index - 1]?.current?.focus();
      }
    }
  };
  return /*#__PURE__*/React.createElement(View, {
    style: styles.container
  }, Array.from({
    length
  }).map((_, index) => /*#__PURE__*/React.createElement(InputNumeric, {
    key: index,
    style: style,
    value: value[index] || '',
    inputRef: refArray[index],
    onChangeText: text => _onChangeText(text, index),
    onKeyPress: e => onKeyPress(e, index)
  })));
}
//# sourceMappingURL=index.js.map