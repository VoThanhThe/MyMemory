...
export const BalanceEVMContext = createContext<
  BalanceEVMContextType | undefined
>(undefined);

// Define the provider
export const BalanceEVMProvider: React.FC<{children: React.ReactNode}> = ({
  children,
}) => {
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      try {
        ...
        const storedChainActive: string | null = await AsyncStorage.getItem(
          chainActive,
        );
        ...
        if (address && storedChainActive) {
          ...
          const provider = new JsonRpcProvider(
            JSON.parse(storedChainActive).rpc,
          );
